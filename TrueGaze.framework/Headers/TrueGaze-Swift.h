...
///
///    // errorCode
///    // - 1000 : Key value is not normal.
///    // - 1002 : Date expires.
///    // - 2001 : The user did not grant camera permissions.
...
///
///        if state == .tracking {
///            print("x : \(xy[0]) y : \(xy[1]), timestamp : \(timestamp)")
///        }
///    }
///
/// }
...
@protocol Receiver
/// Method is called when SessionHandler object is created successfully and ready to be used.
- (void)onInitialized;
/// Method is called when SessionHandler object is <em>NOT</em> created normally.
/// \param errorCode 1000(The key value is not normal), 1001(App Package name is different from registered name), 1002(When the license period is expired), 2001(The user did not grant camera permissions).
///
...
///
/// \param timestamp Timestamp on camera (in seconds)
///
- (void)onGazeWithXy:(NSArray<NSNumber *> * _Nonnull)xy state:(enum GazeState)state timestamp:(double)timestamp;
@optional
/// Method is called when calibration process is started.
- (void)onCalibrationStarted;
/// Method  is called when calibration process is finished.
- (void)onCalibrationFinished;
/// Method is called when camera image is received.
...
/// Method  is called when eyes are closed for a long time
...
/// </p>
/// Method is called on the portrait mode when look upper side.
...
/// </p>
/// Method is called on the portrait mode when look middle area.
...
/// </p>
/// Method is called  on the portrait mode when look down side.
...
/// </p>
/// Method is called on the landscape mode when look left side.
...
/// </p>
/// Method is called on the landscape mode when look middle area.
...
/// </p>
/// Method is called on the landscape mode when look right side.
...
/// note:
/// You can get the key from https://truegaze.visual-camp.com/
...
/// A configuration that set up eye tracking. <br>
